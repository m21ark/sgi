...
import {
  ...
  DataTexture,
  LinearFilter,
  MathUtils as _Math, // Use MathUtils instead of Math
  Mesh,
  NearestFilter,
  NoToneMapping,
  OrthographicCamera,
...
export class Fire extends Mesh {
  constructor(geometry, options) {
    ...
    this.addSource = function (
      ...
      var endX = Math.min(
        Math.floor((u + radius) * textureWidth),
        textureWidth
      );
      var endY = Math.min(
    ...
    if (
      !_Math.isPowerOfTwo(textureWidth) ||
    ...
    this.configShaders = function (dt) {
      ...
      this.diffuseMaterial.uniforms["swirl"].value = Math.exp(
        this.swirl * -0.1
      );
      this.diffuseMaterial.uniforms["drag"].value = Math.exp(this.drag * -0.1);
      this.diffuseMaterial.uniforms["burnRate"].value =
        this.burnRate * dt * 0.01;
    ...
    this.renderProject = function (renderer) {
      ...
      for (var i = 0; i < 20; i++) {
        ...
        this.projMaterial2.uniforms["densityMap"].value =
          this.fieldProj.texture;
      }
